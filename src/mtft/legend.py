...
# ═══════════════════════════════════════════════════════════════
#  v0.33.0 (2026-09-23) — the theorem compendium's exact inputs and
#  corrections applied to the toolset (CC-33; D4 method note)
# ═══════════════════════════════════════════════════════════════

V0330_LEGEND = (
    dict(name="cc33_tensor_coefficient", tier="2", kind="identity", primitives=("II",), tag="Pr", exactness="EXACT",
         nature="The self-dual tensor's p2 coefficient is -7/360 (28 Weyl units; the +1/360 of R2C-03 was its p1^2 coefficient): cancelling "
                "the p2 term needs n_grav = 0 mod 28 (CC-33; supersedes r2c03_anomaly_lift's n_grav/4 rule). None of M1's ledger-preserving "
                "assignments 24/22/18/16 survives; pure-gaugino rule dim(adj) = 0 mod 28 fails for E6, E7, E8. Witnesses: the genera, 273 = 245 + 28, 1/48 = 30/1440.",
         example="mtft.research.gravitational_anomaly.tensor_coefficient_witnesses()", upstream=("r2c03_anomaly_lift",), ref="docs/SM/CC33_TENSOR_COEFFICIENT.md"),
    dict(name="x0143_gonality_bound", tier="1", kind="identity", primitives=("II", "V"), tag="Pr", exactness="EXACT",
         nature="#X0(143)(F_2) = 4 and #X0(143)(F_4) = 18 from the certified T_2 (Eichler–Shimura), matched by the supersingular count 14 + 4 cusps; "
                "a g^1_e with e <= 3 would force <= 15 points, so X0(143) is neither hyperelliptic nor trigonal (gonality >= 4) and h^0(O(D)) = 1 "
                "for every effective D of degree <= 3 (h^0(O(sum P)) = 1 proved); second route: the canonical ideal is generated by quadrics (Petri). "
                "h^0(O(2 sum P)) = 1 is proved by w13_quotient_petri.",
         example="mtft.hecke.gonality_lower_bound()", upstream=("genus_13", "X0_143"), ref="docs/SM/V0330_COMPENDIUM_NOTES.md"),
    dict(name="w13_quotient_petri", tier="1", kind="identity", primitives=("II", "V"), tag="Pr", exactness="EXACT",
         nature="The genus-6 quotient Y = X0(143)/W13 has canonical ideal generated by its 6 quadrics (cubic products rank 31, quartic 91, mod two "
                "primes and over Q): by Petri, Y is neither hyperelliptic, trigonal nor a plane quintic, so h^0(O_Y(D)) = 1 for effective deg D <= 3. "
                "Since pi^* O_Y(Q1+Q2+Q3) = O_X(2 sum P) and the anti-invariant part is fixed by x0143_gonality_bound, h^0(O_X(2 sum P)) = 1: "
                "M1's massive polarisation multiplicity 7 = 6 + 1 is exact. Closes the last open value of R2C-05.",
         example="mtft.canonical.gates.gate_petri_w13_quotient()", upstream=("x0143_gonality_bound",), ref="docs/SM/V0330_COMPENDIUM_NOTES.md"),
    dict(name="vector_higgs_kodaira_form", tier="2", kind="identity", primitives=("V", "II"), tag="Pr", exactness="EXACT",
         nature="Both polarisations of a flux block as pointwise identities for every metric: m^2 = 2 d*d - |B| (tachyonic, (0,1)-forms of L_neg) "
                "and 2 dbar*dbar + |B| (massive); the Gaussian curvature cancels against the curvature of Lambda^{0,1}. Checked in closed form on "
                "sphere (Brandt–Neri–Coleman), flat torus (Nielsen–Olesen) and genus 13; the R2C-02 formula is restricted to d < 0 (CC-30).",
         example="mtft.research.vector_higgs.kodaira_forms(-6)", upstream=("vector_higgs_tachyon", "flux_block_polarisations"), ref="docs/SM/V0330_COMPENDIUM_NOTES.md"),
    dict(name="surface_higgs_slope_level", tier="2", kind="identity", primitives=("V", "II"), tag="Pr", exactness="EXACT",
         nature="On X0(143) x 143a1 with the product connection the lowest Higgs level is 2 pi mu(L_H)/(A_X A_E): massless exactly on the slope-free "
                "locus (A_X = 2 A_E for S1, A_E/2 for S2), tachyonic for mu < 0 — R2C-07's 'electroweak scale = Kaehler deviation' with a proof.",
         example="mtft.research.product_surface.higgs_slope_mass((-4, 2), (1, 0))", upstream=("product_surface_decoupling", "vector_higgs_kodaira_form"), ref="docs/SM/V0330_COMPENDIUM_NOTES.md"),
    dict(name="up_mass_rank_kunneth", tier="2", kind="identity", primitives=("V", "IV"), tag="Pr", exactness="EXACT",
         nature="rank M <= min(3, 2 * kunneth_rank(v)) for the leading-order up mass matrix (u^c curve factor h^0(S0(P)) = 2): unconditional rank <= 2 "
                "in S2 (curve Higgs factor one-dimensional), while in S1 a Kuenneth-rank-two VEV reaches rank 3; qualifies theta_torus_factor_143a1's "
                "'m_u = 0 at leading order' to Kuenneth-rank-one VEVs in S1.",
         example="mtft.research.theta_torus.up_mass_rank_examples()", upstream=("theta_torus_factor_143a1",), ref="docs/SM/V0330_COMPENDIUM_NOTES.md"),
    dict(name="theta_torus_closed_form", tier="2", kind="identity", primitives=("I", "V"), tag="Cert", exactness="CERTIFIED (two routes, 1e-25)",
         nature="Torus factor B_{j,b} = c_j(b) (2k/((k+1) Im tau))^(1/4) from the theta multiplication formula and the closed-form norms (Im tau/2k)^(1/2); "
    ...
    dict(name="lie_closure_svd_gate", tier="3", kind="definition", primitives=("III",), tag="Cert", exactness="CERTIFIED (method note)",
         nature="Lie closure by singular-value rank with a spectral-gap gate (close_lie_svd): no small residual is ever normalised; the D4 closure reads "
                ...
                "on one platform. Method change, not a change of result.",
