...
def cross_map_readout(sector: Dict, H: sp.Matrix, maps: Dict) -> Dict:
    """K_ij = J_i♯ H J_j along the q-maps; α = Tr(R♯(K₀₁−K₁₀)) / (320 Tr(R♯R)) in the (13,17) normalisation
    generalises to Tr(R♯(K₀₁−K₁₀)) / (c Tr(R♯ R)) with c read from the family."""
...
def two_objectives(sector: Dict, alpha=None) -> Dict:
    ...
    return {"least_leakage_trace": sp.expand(least_leakage(sector, H, maps)),
            ...
            "readout_alpha": sp.simplify(cross_map_readout(sector, H, maps)["R_pairing"] / (320 * cross_map_readout(sector, H, maps)["R_norm2"]))}
