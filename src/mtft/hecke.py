...
__all__ = [
    "point_counts_from_T2", "supersingular_lower_bound", "gonality_lower_bound",
...
# ---------------------------------------------------------------------------
# v0.33.0 (2026-09-23): point counts over F_2, F_4 and the gonality bound
# (theorem compendium, Lemma E.1).  EXACT.
# ---------------------------------------------------------------------------

def point_counts_from_T2():
    """#X_0(143)(F_2) and #X_0(143)(F_4) from the certified T_2 (Eichler–Shimura).

    For a good prime p, #X(F_{p^m}) = p^m + 1 - sum_f (alpha_f^m + conj^m)
    over a Hecke eigenbasis f of S_2(Gamma_0(N)), with alpha + conj = a_p(f)
    and alpha * conj = p.  On the 26-dimensional cuspidal homology every
    eigenvalue appears twice, so the traces on S_2 are half those on H_1.
    With m = 1 the sum is tr T_2; with m = 2 it is tr T_2^2 - 2 p dim S_2.
    Result: 4 (the four cusps) and 18.  EXACT.
    """
    C = _as_list(cuspidal_hecke(2))
    n = len(C)
    dim_S2 = n // 2
    tr1 = sum(C[i][i] for i in range(n)) / 2
    tr2 = sum(C[i][k] * C[k][i] for i in range(n) for k in range(n)) / 2
    N2 = 2 + 1 - tr1
    N4 = 4 + 1 - (tr2 - 2 * 2 * dim_S2)
    assert N2.denominator == 1 and N4.denominator == 1
    return {"dim_S2": dim_S2, "tr_T2": tr1, "tr_T2_squared": tr2,
            "F2": int(N2), "F4": int(N4)}


def supersingular_lower_bound(p=2, N=143, cusps=4):
    """Second route to #X_0(N)(F_{p^2}): the supersingular points are
    F_{p^2}-rational and their number weighted by 1/|Aut| is
    (p - 1) psi(N)/24 (Eichler mass formula); every weight is at most 1/2,
    so there are at least (p - 1) psi(N)/12 of them, plus the rational cusps.
    For N = 143, p = 2: psi = 168, mass 7, at least 14 + 4 = 18 points."""
    psi = N
    for q in sorted({f for f in range(2, N + 1) if N % f == 0 and all(f % r for r in range(2, f))}):
        psi = psi * (q + 1) // q
    mass = Fr((p - 1) * psi, 24)
    return {"psi": psi, "mass": mass, "supersingular_at_least": int(2 * mass), "with_cusps_at_least": int(2 * mass) + cusps}


def gonality_lower_bound():
    """X_0(143) is neither hyperelliptic nor trigonal: gonality >= 4.

    A g^1_e with e <= 3 is unique on a curve of genus >= 5 (Castelnuovo–
    Severi), hence Galois-stable and, X having a rational point, defined
    over Q; its reduction mod 2 (good reduction) is a map of degree <= 3
    to P^1 over F_2, which would force #X(F_4) <= 3 * 5 = 15.  But
    #X(F_4) = 18 (`point_counts_from_T2`, confirmed by
    `supersingular_lower_bound`).  Consequence: h^0(O(D)) = 1 for every
    effective divisor D of degree <= 3 — in particular h^0(O(P + Q)) = 1
    and h^0(O(P1 + P2 + P3)) = 1 (used by the polarisation multiplicities,
    the S2 Higgs count and the up-mass rank analysis).  Second route: the
    canonical ideal of X is generated by quadrics (`canonical.gates.
    gate_generation`, P7 = P8 = 395), which by Petri also excludes a g^1_3.
    Nothing here is implied for degree 6; h^0(O(2 sum P)) = 1 is proved
    separately by `canonical.gates.gate_petri_w13_quotient` (the genus-6
    quotient X0(143)/W13 has gonality >= 4)."""
    pc = point_counts_from_T2()
    ss = supersingular_lower_bound()
    e_max = max(e for e in range(1, 20) if e * 5 < pc["F4"])
    return {"F4": pc["F4"], "two_routes_agree": ss["with_cusps_at_least"] <= pc["F4"],
            "excluded_degrees": [e for e in range(2, e_max + 1)], "gonality_at_least": e_max + 1,
            "h0_one_for_effective_degree_up_to": e_max}
